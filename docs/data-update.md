...
どちらも処理は「**各県を並列 fetch → アーティファクト化 → 集約ジョブが全県分の差分を1本に
まとめて単一 PR を自動作成**」まで（`main` へは直接 push しない）。PR をマージして反映する。
...
2. `fetch`（県ごとの matrix, `max-parallel: 5`）:
   ...
   7. 変更後の `data/{pref}.json` / `{pref}_wards.json` をアーティファクト `data-{pref}` に
      アップロード（main へは push しない）
3. `open-pr`（`needs: fetch`, `if: always()`）: 全アーティファクトを回収して `data/` に展開し、
   全県分の差分を1コミットにまとめて単一 PR を作成（一部県が失敗しても成功分は PR に載る）。
   差分が無ければ PR は作らない。
...
>   → **fetch（並列・アーティファクト化）→ 単一 open-pr ジョブが集約して1本の PR を作成**する
>   方式に変更し、書き込みを1ジョブに集約してレースを構造的に排除。
...
2. **反映は単一 PR の集約方式**
   各県の fetch 結果はアーティファクト化し、`open-pr` ジョブが全県分を1コミットにまとめて
   PR（`data/annual-{run_id}` 等）を作成する。`main` への直 push は行わない。PR をマージして反映。
   bot の PR 作成には `permissions: pull-requests: write` と `GITHUB_TOKEN`（`gh` CLI）を使用。
