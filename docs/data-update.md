...
2. **コミット先は `main` 直（並列 push のレース対策あり）**
   github-actions[bot] が `git push origin HEAD:main` する。`max-parallel`（annual=5 /
   quarterly=3）で複数県が同時に push するため、commit ステップは
   **`git pull --rebase` → `git push` を最大8回リトライ**（ジッタ付き sleep）して
   non-fast-forward 競合を吸収する。全リトライ失敗時のみステップを失敗させる。
   ブランチ運用に変えたい場合は両ワークフローの commit ステップを要変更。
